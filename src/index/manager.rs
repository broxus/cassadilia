...
use super::persistence::{IndexStatePersister, PersisterError};
use super::state::{IndexState, IndexStateError, IndexStateItem};
    ...
    K: KeyBytes + Clone + Eq + Ord + std::hash::Hash + Debug + Send + Sync + 'static,
{
    pub fn load(db_root: PathBuf, config: Config) -> Result<Self, IndexError> {
        ...
        let index_file_size = std::fs::metadata(paths.index_file_path()).map_or(0, |m| m.len());
